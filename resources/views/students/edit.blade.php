            ...
            <div class="col-12 col-lg-9 col-xl-7">
                <div class="card shadow-2-strong card-registration" style="border-radius: 15px;">
                    <div class="card-body p-4 p-md-5">
                        ...
                        <form action="{{ route('students.update', $student->id) }}" method="post" id="register-form">
                            @csrf
                            ...
                            <div class="mb-3">
                                ...
                                <select name="classroom_id" style="margin-bottom: 20px !important;" class="custom-select mr-sm-2" >
                                    @foreach($classrooms as $classroom)
                                        <option value="{{ $classroom->id }}" @if($student->classroom->id == $classroom->id) selected @endif>{{ $classroom->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-3 col-md-6 mb-4">
                                ...
                                @foreach(\App\Models\Student::GENDER_MAP as $key => $gender)
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="gender" id="{{ 'gender' . $key }} value="{{ old('gender', $student->gender) }}"
                                               value="{{ $key }}" />
